extends Node3D
## Minimal Watcher presentation: mirrors sim position, no AI/behavior yet (Phase D
## step 7 scope — Watcher exists only to prove the content pipeline + damage matrix +
## Burn contact-spread against a real target). Never mutates sim state (Prime
## Directive 1). TargetBody (a StaticBody3D on the dedicated "aimable_targets" physics
## layer) exists purely so the Envoy's mouse-aim raycast can find this enemy — it
## carries no gameplay collision response of its own. Structural mirror of fang.gd.
